  ...
  const LoginButton = () => {
    ...
    return (
        <div className="w-full grid justify-center align-middle">
            <div
                className="bg-sptf hover:bg-black-600 w-60 rounded-full text-black font-medium px-1 py-1 flex cursor-pointer justify-center align-middle"
                onClick={() => {window.location = spotify_url}}>
                <i className="m-2 fab fa-spotify"></i>
                <button className="my-1">LOG IN WITH SPOTIFY</button>
            </div>
        </div>
    );
  }
    ...
    <div className="bg-dark_main min-h-screen p-5">
      ...
      {(access_token) && (
        <div className="w-full">
            ...
            className="bg-black text-white px-2 py-1 rounded-bl rounded-tl w-80 mb-3"
            placeholder="Search music"></input>
            <button onClick={() => {handleClick()}} className="bg-sptf bg-gray-600 px-2 py-1 mb-3 text-white rounded-br rounded-tr"><i className="fa fa-search"></i></button>
        </div>
      )}
